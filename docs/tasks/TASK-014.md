# TASK-014 — Order Block Engine

## Objective

Implement causal displacement + structure-event Order Blocks.

## Dependencies

- TASK-013
- TASK-008

## Required Reading

- `../PRD.md`
- `../SMC_RULES.md`
- `../ARCHITECTURE.md`
- `../DATA_MODELS.md`
- `../CODING_GUIDELINES.md`

## Implementation Checklist

- [ ] Require qualifying displacement and BOS/CHoCH.
- [ ] Find last opposite candle.
- [ ] Full candle range V1.
- [ ] ACTIVE/MITIGATED/INVALIDATED.
- [ ] Close-based invalidation.

## Acceptance Criteria

- [ ] Random opposite candle not OB.
- [ ] Causal structure event stored.
- [ ] State tests pass.

## Verification

Run repository-equivalent commands for:

```text
typecheck / svelte-check
lint
unit tests
```

## Scope Guard

Implement TASK-014 only.

Do not opportunistically implement later realtime/UI tasks.

Do not add live trading execution or trading credentials.

## Completion Report

Report:

1. files changed;
2. behavior implemented;
3. tests added;
4. commands/results;
5. assumptions;
6. unresolved ambiguities;
7. lookahead-bias review.
